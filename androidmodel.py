# -*- coding: utf-8 -*-
"""
Created on Wed Sep  9 04:44:18 2020
...
class AndroidModel:
    ...
    def decode_outputs(self, target_words_vocab, target_input, batch_size, batched_contexts, valid_mask,
        ...
        return outputs, final_states, contexts_average
    ...
    def build_test_graph(self, input_tensors):
        ...
        with tf.variable_scope('model', reuse=self.get_should_reuse_variables()):
            ...
            batched_contexts = self.compute_contexts(subtoken_vocab=subtoken_vocab, nodes_vocab=nodes_vocab,
                                                     ...
                                                     path_lengths=path_lengths, path_target_lengths=path_target_lengths,
                                                     is_evaluating=True)
            print("writing first graph def")
            tf.io.write_graph( self.sess.graph_def, 'C:\\Users\\philippe\\python-projects\\code2seq\\data\\graphs', "encoder.txt")
            outputs, final_states, method_embedding = self.decode_outputs(target_words_vocab=target_words_vocab,
                                                        target_input=target_index, batch_size=tf.shape(target_index)[0],
                                                        batched_contexts=batched_contexts, valid_mask=valid_mask,
                                                        is_evaluating=True)
            print("writing second graph def")
            tf.io.write_graph( self.sess.graph_def, 'C:\\Users\\philippe\\python-projects\\code2seq\\data\\graphs', "fullgraph.txt")
        if self.config.BEAM_WIDTH > 0:
        ...
        return predicted_indices, topk_values, target_index, attention_weights, method_embedding
