...
X[,"lvefbin"] <- NULL
y  <- d$lvefbin
for(numberOfTrees in c(10, 50, 100, 250, 500, 1000)) {
  for(maxNodes in c(5, 10, 20, 50)) {
    model <- randomForest(X, y, method="class", data=d, ntree = numberOfTrees, maxnodes = maxNodes)  
    #
    # erreur d'apprentissage
    #
    pred <- predict(model, newdata = d, type="vote")
    pred <- pred[,"good"] > 0.5
    pred[pred == TRUE] <- "good"
    pred[pred == FALSE] <- "bad"
    erreur_apprentissage <- error_rate(d$lvefbin, pred)
    print(paste(c("erreur_apprentissage=",erreur_apprentissage, ", maxNode = ", maxNodes, ", numberOfTrees = ", numberOfTrees), ""))
  }
}

model <- randomForest(X, y, method="class", data=d, ntree = 100, maxnodes = 50)  
...
read_pred("20181126_grid_search_rf_pred.csv")